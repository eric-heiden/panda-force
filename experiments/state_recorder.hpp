...
struct StateRecorder
{
    ...
    void process()
    {
        for (size_t i = 0; i < states.size(); ++i)
        {
            ...
            log["EE_wrench"].push_back({wrench[0], wrench[1], wrench[2], wrench[3], wrench[4], wrench[5]});

            Eigen::Vector3d linear_force = wrench.segment(0, 3);
            log["EE_linear_force"].push_back({linear_force[0], linear_force[1], linear_force[2]});

            Eigen::Matrix<double, 6, 1> twist = jacobian * dq;
            log["EE_twist"].push_back({twist[0], twist[1], twist[2], twist[3], twist[4], twist[5]});
        }
    }
